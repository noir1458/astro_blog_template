...
const publicPath = nonEmptyString
  .refine(
...
const languageDefinition = z
  .object({
    label: nonEmptyString,
    locale: nonEmptyString,
    ogLocale: z.string().regex(/^[A-Za-z]{2,3}_[A-Za-z]{2,4}$/u, "must look like ko_KR or en_US"),
    pathPrefix: z
      .string()
      .refine(
...
const giscusSchema = z
  .object({
    repo: nonEmptyString.regex(/^[^/\s]+\/[^/\s]+$/u, "must look like owner/repository"),
...
export const navigationFileSchema = z
  .object({
...
export const socialFileSchema = z
  .object({
...
const categorySlug = nonEmptyString
  .regex(
