...
test("the repository configuration is valid", () => {
  ...
  assert.deepEqual(config.supportedLanguageCodes, ["en", "ko", "ja"]);
...
test("duplicate language path prefixes are rejected", () => {
  const directory = configFixture();
  replaceInFile(directory, "site.yaml", "pathPrefix: /ja", "pathPrefix: /ko");

  assert.throws(
    () => loadSiteConfig({ configDirectory: directory }),
    (error) =>
      error instanceof SiteConfigError
      && /languages\.ja\.pathPrefix: duplicates languages\.ko\.pathPrefix/u.test(error.message)
  );
});
...
test("comments require a complete Giscus configuration", () => {
  const directory = configFixture();
  replaceInFile(directory, "features.yaml", "comments: false", "comments: true");
