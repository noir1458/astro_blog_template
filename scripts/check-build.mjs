...
const expectedOgImage = absoluteSiteUrl(SITE.socialImage);
...
const expectedIndexCanonical = absoluteSiteUrl("/");
...
if (FEATURES.rss) {
  ...
  if (!rssFeed.includes(`<link>${escapeXml(absoluteSiteUrl("/"))}</link>`)) {
    errors.push("rss.xml: configured site URL is missing");
  }
...
if (!aboutHtml.includes(`src="${sitePath(SITE.author.profileImage)}"`)) {
...
if (manifest) {
  ...
  if (manifest.start_url !== sitePath("/") || manifest.scope !== sitePath("/")) {
    errors.push(`manifest.webmanifest: expected base path ${sitePath("/")}`);
  }
  if (manifest.icons?.[0]?.src !== sitePath(SITE.manifestIcon)) {
...
if (FEATURES.sitemap) {
  const sitemap = walk(distRoot, (file) => /^sitemap-.*\.xml$/u.test(path.basename(file)))
    ...
    .join("\n");
  for (const entry of contentEntries) {
    const expectedLocation = `<loc>${absoluteSiteUrl(postPath(entry))}</loc>`;
